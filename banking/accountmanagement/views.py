    ...
    def post(self,request):
            ...
            try:
                ...
                try: 
                    ...
                    account = Account.objects.create(user=user,acc_type=acc_type,acc_no=acc_no)
          ...
          if account.acc_balance!=0.0:
              return Response("please withdraw the money")
                ...
                if account.acc_status=="closed":
                     return Response({"status":"Account Already Closed"}, status=status.HTTP_400_BAD_REQUEST)
                else:   
            ...
            return Response({"status": "account doestnot exist"}, status=status.HTTP_404_NOT_FOUND)  
...
class AccountApproval(APIView):
      ...
      def patch(self,request):
        try: 
            ...
            return Response({"status": "account doestnot exist"}, status=status.HTTP_400_BAD_REQUEST) 
